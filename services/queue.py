...
class QueueService:
    ...
    async def add_to_queue(item: QueueItem) -> bool:
        ...
        try:
            ...
            queue_length = await mongodb.db.queue_items.count_documents({
            ...
            await mongodb.db.queue_items.insert_one(item_dict)
    ...
    async def get_queue(chat_id: int) -> List[QueueItem]:
        ...
        try:
            ...
            items = await mongodb.db.queue_items.find({
                'chat_id': chat_id,
                'played': False
    ...
    async def get_current_item(chat_id: int) -> Optional[QueueItem]:
        ...
        try:
            ...
            item = await mongodb.db.queue_items.find_one({
                'chat_id': chat_id,
                'played': True
            }, sort=[('requested_at', -1)])
        ...
        try:
            ...
            async with await mongodb.client.start_session() as session:
                async with session.start_transaction():
                    # Find the next item
                    item = await mongodb.db.queue_items.find_one_and_update(
                    ...
                    if item:
                        ...
                        await mongodb.db.queue_items.update_many(
                            {
                                'chat_id': chat_id,
                                'played': False,
    ...
    async def clear_queue(chat_id: int) -> bool:
        ...
        try:
            ...
            result = await mongodb.db.queue_items.delete_many({'chat_id': chat_id})
        ...
        try:
            ...
            async with await mongodb.client.start_session() as session:
                async with session.start_transaction():
                    ...
                    result = await mongodb.db.queue_items.delete_one(
    ...
    async def get_queue_length(chat_id: int) -> int:
        ...
        try:
            ...
            return await mongodb.db.queue_items.count_documents({
                'chat_id': chat_id,
                'played': False
            })
        except Exception as e:
            ...
            async with await mongodb.client.start_session() as session:
                async with session.start_transaction():
                    ...
                    for idx, item in enumerate(items, 1):
                        await mongodb.db.queue_items.update_one(
                            {'_id': item['_id']},
                            {'$set': {'position': idx}},
                            session=session
                        )
